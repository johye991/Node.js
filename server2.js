...
POST : 보내다는 의미 (클라이언트가 서버에 데이터를 전송할때 사용하는 방식)
         ...
         는것과 같다, 봉투 안에 담아서 보내기 때문에 데이터 노출되지 않는다..
         특징) 보낼수 있는 데이터량에 한계가 없다. 
         용도) 노출되면 위험한 데이터 전송시 사용(로그인요청, 회원가입 등...)
                 파일(사진,동영상 등) 등을 서버에 전송시

        post방식은 form 태그를 이용해서 요청할 수 있다..                 
*/
var http = require("http");
var url = require("url");
var querystring=require("querystring"); //get , post 까지 파싱이 가능
